...
export default function ContributorSection({ userId }: { userId: string }) {
  ...
  return (
    <div className="mt-6 rounded-2xl bg-white dark:bg-gray-900 shadow-sm p-4 space-y-4">
      <div className="flex items-center justify-between border-b pb-4" style={{ marginBottom: 20, paddingBottom: 10 }}>
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Contributor Profile</h3>
      ...
      <div className="flex gap-4 items-start">
        {contributor.profileImage ? (
          <div className="h-16 w-16 shrink-0 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-800 relative">
            <Image
          ...
          <div className="h-16 w-16 shrink-0 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center border border-gray-200 dark:border-gray-800 text-xl font-semibold text-gray-500 dark:text-gray-400">
      ...
      {contributor.bio && (
        <div className="pt-2">
          <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{contributor.bio}</p>
        </div>
      )}
      ...
      {contributor.category && contributor.category.length > 0 && (
        <div className="pt-2 flex flex-wrap gap-2">
          {contributor.category.map((cat, idx) => (
            <span
              key={idx}
              className="px-2.5 py-1 text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-full capitalize"
